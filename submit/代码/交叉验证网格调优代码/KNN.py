...
model=GridSearchCV(KNeighborsClassifier(),param_grid=param,n_jobs=-1,scoring='f1',cv=5,refit=True)
...
np.savetxt("KNN调参-results.csv", res, delimiter=',', header='precision,recall,f1-score,auc-value')
np.savetxt("KNN调参-AUC-fpr.csv", fpr, delimiter=',')
np.savetxt("KNN调参-AUC-tpr.csv", tpr, delimiter=',')
np.savetxt("KNN-参数.csv",canshu,delimiter=',',fmt='%s')
