...
param = {
    ...
    'min_child_weight': [1],
    'max_delta_step': [100],
    'objective':['reg：linear','binary:logitraw']
}
model = GridSearchCV(XGBClassifier(random_state=1), param_grid=param, n_jobs=-1, scoring='f1', cv=5, refit=True)
...
np.savetxt("XGBoost调参-results.csv", res, delimiter=',', header='precision,recall,f1-score,auc-value')
np.savetxt("XGBoost调参-AUC-fpr.csv", fpr, delimiter=',')
np.savetxt("XGBoost调参-AUC-tpr.csv", tpr, delimiter=',')
np.savetxt("XGBoost-参数.csv", canshu, delimiter=',', fmt='%s')
