...
model=GridSearchCV(GradientBoostingClassifier(random_state=1),param_grid=param,n_jobs=-1,scoring='f1',cv=5,refit=True)
...
np.savetxt("GBDT调参-results.csv", res, delimiter=',', header='precision,recall,f1-score,auc-value')
np.savetxt("GBDT调参-AUC-fpr.csv", fpr, delimiter=',')
np.savetxt("GBDT调参-AUC-tpr.csv", tpr, delimiter=',')
np.savetxt("GBDT-参数.csv",canshu,delimiter=',',fmt='%s')
